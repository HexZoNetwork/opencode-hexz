...
const VERSION = "1.5.2"
const memoryPath = join(homedir(), ".config", "claude", "hexz-memory.json")
const SIMS_DIR = ".sims"
...
function generateClaudePluginManifest(): string {
...
function generateClaudeSkill(name: string, description: string, prompt: string): string {
  return `---
name: ${name}
...
function generateClaudeAgent(): string {
  return `---
name: hexz-agent
...
function generateClaudePluginDir(baseDir: string): void {
  const pluginDir = join(baseDir, ".claude-plugin")
  ...
  writeFile(join(pluginDir, "plugin.json"), generateClaudePluginManifest())
  ...
  writeFile(join(agentsDir, "hexz-agent.md"), generateClaudeAgent())
  ...
  for (const skill of skills) {
    ...
    writeFile(join(skillDir, "SKILL.md"), generateClaudeSkill(skill.name, skill.description, skill.prompt))
  }
}
...
export const hexz_mcp = tool({
  ...
  async execute(args: any) {
    const a = args.action
    if (a === "list") {
      const configPath = join(homedir(), ".config", "claude", "mcp-servers.json")
      try {
    ...
    if (a === "connect") {
      ...
      try { Object.assign(servers, JSON.parse(readFileSync(join(homedir(), ".config", "claude", "mcp-servers.json"), "utf8"))) } catch {}
      const name = new URL(args.server).hostname || `server-${Date.now()}`
      servers[name] = { url: args.server }
      mkdirSync(join(homedir(), ".config", "claude"), { recursive: true })
      writeFileSync(join(homedir(), ".config", "claude", "mcp-servers.json"), JSON.stringify(servers, null, 2) + "\n")
...
export const hexz_mkp = tool({
  ...
  async execute(args: any) {
    ...
    const toolsDir = join(process.cwd(), ".claude-plugin")
    ...
    if (t.startsWith("skill:")) {
      ...
      if (skillTarget.includes("--content")) {
        ...
        mkdirSync(skillDir, { recursive: true })
        writeFile(join(skillDir, "SKILL.md"), generateClaudeSkill(name, "Custom skill", content))
...
export const hexz_status = tool({
  ...
  execute() {
    ...
    return `HEXZ for Claude Code | v${VERSION} | ${Math.floor(up / 60)}m${up % 60}s | cache: ${searchCache.size} entries${fallback}`
  },
})
...
export const hexz_cyber = tool({
  ...
  async execute(args: any) {
    const cyberDir = (() => {
      const candidates = [join(process.cwd(), ".claude-plugin", "cybersecurity"), join(process.cwd(), "src", "cybersecurity")]
...
export async function initClaudePlugin(input: any = {}): Promise<Record<string, any>> {
  const projectDir = input?.directory ?? process.cwd()
  generateClaudePluginDir(projectDir)
  return {
...
export default initClaudePlugin
export const server = initClaudePlugin
export const plugin = initClaudePlugin
