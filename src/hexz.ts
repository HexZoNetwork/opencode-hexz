...
function guessTopic(cmd: string): string {
  ...
  const tech: Record<string, string> = {
    ...
    go: "Go golang", rust: "Rust", python: "Python", typescript: "TypeScript",
    javascript: "JavaScript", java: "Java", ruby: "Ruby", php: "PHP",
    swift: "Swift", kotlin: "Kotlin", csharp: "C#",
    docker: "Docker", kubernetes: "Kubernetes", k8s: "Kubernetes",
    postgres: "PostgreSQL", postgresql: "PostgreSQL", mysql: "MySQL",
    redis: "Redis", mongodb: "MongoDB", graphql: "GraphQL", rest: "REST API",
    grpc: "gRPC", websocket: "WebSocket", jwt: "JWT", oauth: "OAuth2",
    api: "API design", auth: "authentication", testing: "testing",
    deploy: "deployment", cicd: "CI/CD", frontend: "frontend",
    backend: "backend", fullstack: "fullstack",
  }
  for (const [k, v] of Object.entries(tech)) {
    if (l.includes(k)) return `${v} best practices ${y}`
  }
...
const BUILD_PATTERNS = [
  ...
  "pip install", "pip3 install", "poetry add",
  "cargo init", "cargo new", "cargo add", "go mod init",
  "git clone", "git init", "mkdir", "touch ",
    ...
    "chat.message": async (_input, output) => {
      ...
      if (text.includes("HEXZ_ACTIVATE") || text.toLowerCase().includes("engage hexz")) {
        ...
        for (const part of output.parts) {
          if ("text" in part) (part as any).text = "[HEXZ] Active. Senior engineer mode."
        }
        return
      }
      ...
      if (text.includes("HEXZ_DEACTIVATE") || text.toLowerCase().includes("revert to default")) {
        state.active = false
        for (const part of output.parts) {
          if ("text" in part) (part as any).text = "[HEXZ] Off."
        }
        return
      }
    },
...
2. Execute: replicate (UI), fix (error), code (diagram), build (mockup)`
      ...
      hexz_mkp: tool({
        get description() {
          return state.active
            ? "Install opencode plugin. 'owner/repo' from GitHub or 'npm-name' via bun."
            : "[HEXZ OFF] Activate with /active first."
        },
        ...
        async execute(args, ctx) {
          ...
          if (!t || !isSafe(t)) return "Invalid target"
          try {
            if (t.includes("/")) {
            ...
            if (!/^[a-zA-Z0-9@._-]+$/.test(t)) return "Invalid package name"
